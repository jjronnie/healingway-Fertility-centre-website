<x-app-layout>
  <div class="max-w-4xl mx-auto bg-white rounded-lg shadow-xl p-8">
        <h1 class="text-3xl font-bold text-gray-800 mb-6 text-center">Edit Doctor: <span class="text-blue-600">{{ $doctor->name }}</span></h1>
        ...
        <form action="{{ route('admin.doctors.update', $doctor) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="position" id="position" value="{{ old('position', $doctor->position) }}"
                       class="shadow appearance-none border rounded-lg w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:shadow-outline focus:border-blue-500 transition duration-200"
            ...
            <div class="mb-4">
                ...
                <textarea name="body" id="body" rows="6"
                          class="shadow appearance-none border rounded-lg w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:shadow-outline focus:border-blue-500 transition duration-200"
                          placeholder="Detailed information about the doctor...">{{ old('body', $doctor->body) }}</textarea>
            </div>

              <div class="mb-4">
                <label for="display_position" class="block text-gray-700 text-sm font-bold mb-2">Display Order:</label>
                <input type="number" name="display_position" id="display_position" value="{{ old('display_position', $doctor->display_position ?? 0) }}"
                       class="shadow appearance-none border rounded-lg w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:shadow-outline focus:border-blue-500 transition duration-200"
            ...
            <div class="mb-6">
                ...
                @if ($doctor->photo)
                    <img src="{{ asset($doctor->photo) }}" alt="{{ $doctor->name }}" class="w-24 h-24 object-cover rounded-full mb-3 shadow-md">

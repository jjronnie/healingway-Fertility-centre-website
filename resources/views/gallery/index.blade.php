<x-guest-layout>
    <x-page-header image="assets/img/1.webp" title="Gallery"
        description="Browse moments from HealingWay Fertility Centre." />
                ...
                <div class="inline-flex items-center gap-3 bg-white/95 border border-gray-200 shadow-sm rounded-full px-4 py-2">
                    <div class="w-44 sm:w-56">
                        <select class="input w-full bg-transparent border-0 focus:ring-0 focus:border-transparent text-sm py-1"
                            ...
                            <option value="{{ route('gallery.index') }}" selected>All</option>
                            @foreach ($categories as $category)
                                <option value="{{ route('gallery.category', $category->slug) }}">
                                    {{ $category->name }}
                                </option>
                            @endforeach
                        </select>
    ...
    <section class="pb-16 -mt-3" x-data="galleryGrid({
        initialItems: @js($latestImages->getCollection()->map(fn($image) => [
            'id' => $image->id,
            'title' => $image->title,
        ...
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="text-center py-6" x-show="!items.length" x-cloak>
                <x-empty-state icon="image" message="No gallery images available yet." />
            </div>
